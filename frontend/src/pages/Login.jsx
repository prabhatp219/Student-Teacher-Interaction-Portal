...
export default function Login() {
  const { login, googleLogin, setPassword: saveNewPassword } = useContext(AuthContext);
  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [err, setErr] = useState("");
  const [showPassword, setShowPassword] = useState(false);
  const [loadingDemo, setLoadingDemo] = useState(null);
  const [pendingFirstLogin, setPendingFirstLogin] = useState(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.isFirstLogin) {
        setPendingFirstLogin({ isGoogle: false, role: result.role });
      } else {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      if (!credentialResponse?.credential) {
        setErr("No credential received from Google.");
        return;
      }
      const result = await googleLogin(credentialResponse.credential);
      if (result.isFirstLogin) {
        setPendingFirstLogin({ isGoogle: true, role: result.role });
      } else {
        redirect(result.role);
      }
    } catch (error) {
      ...
      const serverMsg =
        ...
        (error.response?.status === 403
          ? "You are not registered with EduHub. Please contact the administrator."
  ...
  const handleGoogleError = () => {
    setErr("Google authentication was cancelled or failed.");
  };

  const handleFirstLoginComplete = async (newPassword) => {
    await saveNewPassword(newPassword);
    const role = pendingFirstLogin?.role || "student";
  ...
  return (
    <div className="login-page">
            ...
            <div className="google-auth-container">
              {googleClientId ? (
                ...
                <button
                  ...
                  onClick={() =>
                    setErr("Google Client ID is not configured. Please add VITE_GOOGLE_CLIENT_ID to your frontend .env")
                  }
                >
                  <span className="google-icon-svg">{icons.google}</span>
      ...
      {pendingFirstLogin && (
        <FirstLoginModal
          isGoogle={pendingFirstLogin.isGoogle}
